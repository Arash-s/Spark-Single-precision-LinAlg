...
import java.lang.{Float => JavaFloat, Integer => JavaInteger, Iterable => JavaIterable}
import java.util
...
/**
 * Represents a numeric vector, whose index type is Int and value type is Float.
 *
...
sealed trait Vector extends Serializable {
   ...
   */
  private[spark] def asBreeze: BV[Float]
   ...
   */
  @Since("2.0.0")
  def apply(i: Int): Float = asBreeze(i)
   ...
   */
  @Since("2.0.0")
  def foreachActive(f: (Int, Float) => Unit): Unit
...
object Vectors {
  ...
  @varargs
  @Since("2.0.0")
  def dense(firstValue: Float, otherValues: Float*): Vector =
    new DenseVector((firstValue +: otherValues).toArray)

  // A dummy implicit is used to avoid signature collision with the one generated by @varargs.
  /**
   * Creates a dense vector from a Float array.
   */
  @Since("2.0.0")
  def dense(values: Array[Float]): Vector = new DenseVector(values)
   ...
   */
  @Since("2.0.0")
  def sparse(size: Int, indices: Array[Int], values: Array[Float]): Vector =
   ...
   */
  @Since("2.0.0")
  def sparse(size: Int, elements: Seq[(Int, Float)]): Vector = {
   ...
   */
  @Since("2.0.0")
  def sparse(size: Int, elements: JavaIterable[(JavaInteger, JavaFloat)]): Vector = {
    sparse(size, elements.asScala.map { case (i, x) =>
      (i.intValue(), x.floatValue())
    }.toSeq)
  }
  ...
  def zeros(size: Int): Vector = {
    new DenseVector(new Array[Float](size))
  }

  /**
   * Creates a vector instance from a breeze vector.
   */
  private[spark] def fromBreeze(breezeVector: BV[Float]): Vector = {
    breezeVector match {
      case v: BDV[Float] =>
  ...
  def norm(vector: Vector, p: Float): Float = {
    require(p >= 1.0F, "To compute the p-norm of the vector, we require that you specify a p>=1. " +
      s"You specified p=$p.")
    ...
    if (p == 1) {
    ...
    } else if (p == Float.PositiveInfinity) {
      var max = 0.0F
      var i = 0
      while (i < size) {
      ...
      var i = 0
      while (i < size) {
        sum += math.pow(math.abs(values(i)), p).toFloat
        i += 1
      }
      math.pow(sum, 1.0F / p).toFloat
    }
  }
   ...
   */
  private[ml] def sqdist(v1: SparseVector, v2: DenseVector): Float = {
    var kv1 = 0
    var kv2 = 0
  ...
  private[ml] def equals(
      ...
      v1Values: Array[Float],
      v2Indices: IndexedSeq[Int],
      v2Values: Array[Float]): Boolean = {
 ...
 */
@Since("2.0.0")
class DenseVector @Since("2.0.0") ( @Since("2.0.0") val values: Array[Float]) extends Vector {

  override def size: Int = values.length

  override def toString: String = values.mkString("[", ",", "]")

  override def toArray: Array[Float] = values

  private[spark] override def asBreeze: BV[Float] = new BDV[Float](values)
  ...
  override def foreachActive(f: (Int, Float) => Unit): Unit = {
    var i = 0
...
object DenseVector {
  ...
  @Since("2.0.0")
  def unapply(dv: DenseVector): Option[Array[Float]] = Some(dv.values)
}
...
class SparseVector @Since("2.0.0") (
    ...
    @Since("2.0.0") val values: Array[Float]) extends Vector {
  ...
  private[spark] override def asBreeze: BV[Float] = new BSV[Float](indices, values, size)
...
object SparseVector {
  @Since("2.0.0")
  def unapply(sv: SparseVector): Option[(Int, Array[Int], Array[Float])] =
